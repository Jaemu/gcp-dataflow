...
/**
 * In this exercise we start processing unbounded streams of input. As in {@link Exercise3} we use
 * fixed windows and event times. In this example we use triggers to generate early and speculative
 * results, as well as {@code .accumulatingFiredPanes()} to do cumulative processing of
 * late-arriving data.
 *
 ...
 * one-hour windows.
 *
 * <p> To demonstrate other windowing options the user scores are calculated using a
...
public class Exercise4 {
  ...
  public static class UserLeaderBoard
    ...
    public PCollection<KV<String, Integer>> apply(PCollection<GameActionInfo> input) {
      ...
      //
      // Compute the user scores since the beginning of time. To do this we will need:
      //
      // 1. Use the GlobalWindows WindowFn so that all events occur in the same window.
      // 2. Specify the accumulation mode so that total score is constantly increasing.
      ...
      return input
          .apply(Window
              ...
              // When windowing on an unbounded stream, there is always the chance that
              // the watermark is wrong and we encounter late data. This method specifies
              // our upper bound on how "late" data should allowed to be before being dropped.
              // It tunes how data is kept around waiting for these late elements.
  ...
  public static class TeamLeaderBoard
    ...
    public TeamLeaderBoard(
        Duration allowedLateness, Duration earlyUpdateFrequency, Duration lateUpdateFrequency,
        Duration windowSize) {
      this.allowedLateness = allowedLateness;
  ...
  public static void main(String[] args) throws Exception {
    ...
    // Extract user/score pairs from the event stream using processing time for regular updates.
    gameEvents
        .apply(new UserLeaderBoard(ALLOWED_LATENESS, EARLY_UPDATE_FREQUENCY))
        // Write the results to BigQuery.
        .apply(new Output.WriteTriggeredUserScoreSums());

    // Extract team/score pairs from the event stream, using windows with early and late updates.
    gameEvents
        .apply(new TeamLeaderBoard(
            ALLOWED_LATENESS, EARLY_UPDATE_FREQUENCY, LATE_UPDATE_FREQUENCY, WINDOW_SIZE))
        // Write the results to BigQuery.
